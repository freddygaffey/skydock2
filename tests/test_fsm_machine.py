...
def test_transition_is_logged_as_fsm_transition(monkeypatch, make_drone_state, make_frame):
    ...
    # fsm passes RAW enums to log_event (schema v2); mission_logging encodes to .name on disk.
    assert kwargs["state_from"] == DroneStateEnum.SCAN
    assert kwargs["state_to"] == DroneStateEnum.GOTO
...
def test_same_state_is_logged_as_fsm_tick(monkeypatch, make_drone_state, make_frame):
    ...
    # fsm passes the RAW enum to log_event (schema v2); encoded to .name on disk.
    assert fsm_mod.log_event.call_args[1]["state"] == DroneStateEnum.SCAN
